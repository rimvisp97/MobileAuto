...
function AppShell() {
  ...
  const totals = useMemo(() => {
    ...
    return { spent: vehicleSpent + donorSpent, revenue: vehicleRevenue + partsRevenue, profit: vehicleRevenue + partsRevenue - vehicleSpent - donorSpent };
  }, [vehicles, partsCars]);

  const activity = useMemo(() => {
    const rows: MoneyRow[] = [];
    vehicles.forEach((vehicle) => {
      rows.push({ id: `${vehicle.id}-purchase`, date: vehicle.createdAt, label: `Nupirktas ${vehicle.make} ${vehicle.model}`, detail: 'Automobilis', amount: -vehicle.purchasePrice, kind: 'out' });
    ...
    partsCars.forEach((car) => {
      ...
      car.parts.filter((part) => part.status === 'sold').forEach((part) => rows.push({ id: part.id, date: part.createdAt, label: `Parduota detalė · ${part.name}`, detail: `${car.make} ${car.model}`, amount: part.price, kind: 'in' }));
    });
  ...
  function openModal(name: typeof modal, id?: string) {
    setModal(name);
  ...
  function togglePart(carId: string, partId: string) {
    setPartsCars((current) => current.map((car) => car.id === carId ? { ...car, parts: car.parts.map((part) => part.id === partId ? { ...part, status: part.status === 'inventory' ? 'sold' : 'inventory' } : part) } : car));
  }
    ...
    <div className="noise min-h-[100dvh] bg-background text-foreground">
      ...
      <main className="min-h-[100dvh] lg:pl-[272px]">
        ...
        <div className="app-shell min-h-[calc(100dvh-72px)] p-5 sm:p-8">
          ...
          {page === 'parts' && <PartsPage partsCars={partsCars} openModal={openModal} togglePart={togglePart} deletePartsCar={deletePartsCar} />}
        </div>
      </main>
      ...
      {modal === 'part' && <PartModal car={partsCars.find((car) => car.id === selectedPartsCarId)} close={() => setModal(null)} save={addPart} />}
...
type DashboardProps = { totals: { spent: number; revenue: number; profit: number }; activity: MoneyRow[]; vehicles: Vehicle[]; partsCars: PartsCar[]; navigate: (path: string) => void };
  ...
  return (
    <section className="mx-auto max-w-[1480px]">
      <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div className="reveal"><p className="mb-3 font-mono-ui text-[11px] uppercase tracking-[0.2em] text-secondary">2024 / gegužė</p><h1 className="text-3xl font-bold tracking-tight sm:text-[38px]">Pagrindinis<span className="text-primary">.</span></h1><p className="mt-2 max-w-md text-sm text-muted-foreground">Tavo dirbtuvių knyga, kur skaičiai kalba tiesiai.</p></div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <MetricCard label="Suprekiauta" value={totals.revenue} hint="automobiliai + dalys" icon={<ArrowUpRight size={18} />} tone="teal" />
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card p-5 shadow-sm reveal reveal-delay-3 sm:p-6"><div className="flex items-center justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Paskutinis rezultatas</p><h2 className="mt-1 text-lg font-bold">Pardavimai</h2></div><CircleDollarSign size={20} className="text-primary" /></div>{soldVehicles.length > 0 ? <div className="mt-5 space-y-4">{soldVehicles.slice(0, 3).map((vehicle) => { const profit = (vehicle.salePrice ?? 0) - vehicleCost(vehicle); return <div key={vehicle.id} className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{vehicle.make} {vehicle.model}</p><p className="text-xs text-muted-foreground">{shortDate(vehicle.soldAt ?? vehicle.createdAt)}</p></div><span className={`font-mono-ui text-sm font-bold ${profit >= 0 ? 'text-secondary' : 'text-destructive'}`}>{profit >= 0 ? '+' : '−'}{money(Math.abs(profit))}</span></div>; })}</div> : <p className="mt-5 text-sm text-muted-foreground">Pirmasis pardavimas dar laukia.</p>}</div>
        </div>
      </div>
    </section>
  );
...
function MetricCard({ label, value, hint, icon, tone }: { label: string; value: number; hint: string; icon: ReactNode; tone: 'orange' | 'teal' | 'dark' }) {
  const styles = tone === 'orange' ? 'bg-primary text-primary-foreground' : tone === 'teal' ? 'bg-secondary text-secondary-foreground' : 'bg-foreground text-background';
...
function VehicleCard({ vehicle, index, openModal, deleteVehicle }: { vehicle: Vehicle; index: number; openModal: (name: 'vehicle' | 'expense' | 'sell', id?: string) => void; deleteVehicle: (id: string) => void }) {
  ...
  return <article className={`lift rounded-xl border border-border bg-card shadow-sm reveal reveal-delay-${Math.min(index + 1, 3)}`} data-testid={`card-vehicle-${vehicle.id}`}><div className="border-b border-border p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div><div className="mb-2 flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${vehicle.status === 'sold' ? 'bg-secondary' : 'bg-primary'}`} /><span className="font-mono-ui text-[10px] uppercase tracking-[0.15em] text-muted-foreground">{vehicle.status === 'sold' ? 'Parduotas' : 'Aktyvus'}</span></div><h2 className="text-xl font-bold">{vehicle.year} {vehicle.make} {vehicle.model}</h2><p className="mt-1 text-sm text-muted-foreground">{vehicle.engine} <span className="mx-1">·</span> {vehicle.fuel} <span className="mx-1">·</span> {vehicle.mileage.toLocaleString('lt-LT')} km</p></div><div className="flex gap-1"><button onClick={() => openModal('vehicle', vehicle.id)} className="rounded-lg p-2 text-muted-foreground hover-elevate" aria-label="Redaguoti automobilį" data-testid={`button-edit-vehicle-${vehicle.id}`}><Pencil size={16} /></button><button onClick={() => deleteVehicle(vehicle.id)} className="rounded-lg p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label="Pašalinti automobilį" data-testid={`button-delete-vehicle-${vehicle.id}`}><Trash2 size={16} /></button></div></div><div className="mt-6 grid grid-cols-3 gap-3"><InfoCell label="Pirkimas" value={money(vehicle.purchasePrice)} /><InfoCell label="Išlaidos" value={money(totalExpenses(vehicle))} /><InfoCell label="Savikaina" value={money(cost)} strong /></div></div><div className="flex flex-wrap items-center justify-between gap-3 bg-muted/35 px-5 py-4 sm:px-6">{vehicle.status === 'sold' ? <div><p className="text-[10px] uppercase tracking-[0.13em] text-muted-foreground">Rezultatas</p><p className={`font-mono-ui text-lg font-bold ${profit >= 0 ? 'text-secondary' : 'text-destructive'}`}>{profit >= 0 ? '+' : '−'} {money(Math.abs(profit))}</p></div> : <div><p className="text-[10px] uppercase tracking-[0.13em] text-muted-foreground">Veiksmas</p><p className="text-sm font-semibold">Dar neparduotas</p></div>}<div className="flex gap-2">{vehicle.status === 'active' && <><button onClick={() => openModal('expense', vehicle.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-2 text-xs font-bold hover-elevate" data-testid={`button-add-expense-${vehicle.id}`}><Wrench size={14} /> Išlaidos</button><button onClick={() => openModal('sell', vehicle.id)} className="inline-flex items-center gap-1.5 rounded-lg bg-foreground px-3 py-2 text-xs font-bold text-background hover:opacity-90" data-testid={`button-sell-vehicle-${vehicle.id}`}><BadgeEuro size={14} /> Parduoti</button></>}</div></div>{vehicle.expenses.length > 0 && <div className="border-t border-border px-5 py-4 sm:px-6"><p className="mb-2 font-mono-ui text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Išlaidų žurnalas</p><div className="space-y-2">{vehicle.expenses.slice().reverse().map((expense) => <div key={expense.id} className="flex items-center justify-between text-sm"><span className="text-muted-foreground">{expense.label}</span><span className="font-mono-ui text-xs">− {money(expense.amount)}</span></div>)}</div></div>}</article>;
}
...
function PartsPage({ partsCars, openModal, togglePart, deletePartsCar }: { partsCars: PartsCar[]; openModal: (name: 'partsCar' | 'part', id?: string) => void; togglePart: (carId: string, partId: string) => void; deletePartsCar: (id: string) => void }) {
  const [query, setQuery] = useState('');
  const filtered = partsCars.filter((car) => `${car.make} ${car.model} ${car.year} ${car.engine}`.toLowerCase().includes(query.toLowerCase()));
  return <section className="mx-auto max-w-[1480px]"><SectionHeader eyebrow="Inventorius / 02" title="Dalys" body="Donorai kelyje į antrą gyvenimą. Parduotų detalių pinigai grįžta į knygą." action={<Plus size={17} />} actionLabel="Pridėti donorą" onAction={() => openModal('partsCar')} /><div className="mb-6 flex items-center gap-3"><label className="relative block max-w-md flex-1"><Search size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Ieškoti donorų..." className="h-11 w-full rounded-lg border border-border bg-card pl-10 pr-3 text-sm outline-none placeholder:text-muted-foreground/70 focus:border-primary focus:ring-2 focus:ring-primary/20" data-testid="input-search-parts" /></label><span className="hidden font-mono-ui text-[10px] uppercase tracking-[0.14em] text-muted-foreground sm:block">{partsCars.length} donorai</span></div>{filtered.length === 0 ? <EmptyState title="Donorų neradome" body="Pridėk automobilį ardymui arba pakeisk paiešką." action={<button onClick={() => openModal('partsCar')} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground" data-testid="button-empty-add-donor"><Plus size={16} /> Pridėti donorą</button>} /> : <div className="space-y-5">{filtered.map((car, index) => <PartsCarCard key={car.id} car={car} index={index} openModal={openModal} togglePart={togglePart} deletePartsCar={deletePartsCar} />)}</div>}</section>;
}

function PartsCarCard({ car, index, openModal, togglePart, deletePartsCar }: { car: PartsCar; index: number; openModal: (name: 'partsCar' | 'part', id?: string) => void; togglePart: (carId: string, partId: string) => void; deletePartsCar: (id: string) => void }) {
  ...
  return <article className={`lift overflow-hidden rounded-xl border border-border bg-card shadow-sm reveal reveal-delay-${Math.min(index + 1, 3)}`} data-testid={`card-parts-car-${car.id}`}><div className="flex flex-col gap-5 border-b border-border p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-accent text-accent-foreground"><CarFront size={21} /></div><div><div className="mb-1 flex items-center gap-2"><span className="font-mono-ui text-[10px] uppercase tracking-[0.15em] text-secondary">Donoras · {shortDate(car.createdAt)}</span></div><h2 className="text-xl font-bold">{car.year} {car.make} {car.model}</h2><p className="mt-1 text-sm text-muted-foreground">{car.engine} <span className="mx-1">·</span> {car.mileage.toLocaleString('lt-LT')} km <span className="mx-1">·</span> pirkta už {money(car.purchasePrice)}</p></div></div><div className="flex items-center gap-2"><button onClick={() => openModal('partsCar', car.id)} className="rounded-lg p-2 text-muted-foreground hover-elevate" aria-label="Redaguoti donorą" data-testid={`button-edit-donor-${car.id}`}><Pencil size={16} /></button><button onClick={() => deletePartsCar(car.id)} className="rounded-lg p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label="Pašalinti donorą" data-testid={`button-delete-donor-${car.id}`}><Trash2 size={16} /></button><button onClick={() => openModal('part', car.id)} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-2 text-xs font-bold text-primary-foreground" data-testid={`button-add-part-${car.id}`}><Plus size={15} /> Pridėti detalę</button></div></div><div className="grid border-b border-border bg-muted/30 sm:grid-cols-3"><div className="border-b border-border px-5 py-3 sm:border-b-0 sm:border-r sm:px-6"><p className="text-[10px] uppercase tracking-[0.13em] text-muted-foreground">Parduota</p><p className="mt-1 font-mono-ui text-sm font-bold text-secondary">{money(sold)}</p></div><div className="border-b border-border px-5 py-3 sm:border-b-0 sm:border-r sm:px-6"><p className="text-[10px] uppercase tracking-[0.13em] text-muted-foreground">Sandėlyje</p><p className="mt-1 font-mono-ui text-sm font-bold">{money(inventory)}</p></div><div className="px-5 py-3 sm:px-6"><p className="text-[10px] uppercase tracking-[0.13em] text-muted-foreground">Detalių</p><p className="mt-1 font-mono-ui text-sm font-bold">{car.parts.length}</p></div></div>{car.parts.length > 0 ? <div className="overflow-x-auto"><table className="w-full min-w-[580px] text-left"><thead><tr className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground"><th className="px-5 py-3 font-mono-ui sm:px-6">Detalė</th><th className="px-3 py-3 font-mono-ui">Kodas</th><th className="px-3 py-3 font-mono-ui">Kaina</th><th className="px-5 py-3 text-right font-mono-ui sm:px-6">Būsena</th></tr></thead><tbody>{car.parts.map((part) => <tr key={part.id} className="border-t border-border/70 hover:bg-muted/30" data-testid={`row-part-${part.id}`}><td className="px-5 py-3.5 text-sm font-semibold sm:px-6">{part.name}</td><td className="px-3 py-3.5 font-mono-ui text-xs text-muted-foreground">{part.code || '—'}</td><td className="px-3 py-3.5 font-mono-ui text-xs">{money(part.price)}</td><td className="px-5 py-3.5 text-right sm:px-6"><button onClick={() => togglePart(car.id, part.id)} className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wide transition-colors ${part.status === 'sold' ? 'bg-secondary/15 text-secondary' : 'bg-primary/15 text-primary'}`} data-testid={`button-toggle-part-${part.id}`}>{part.status === 'sold' ? <Check size={12} /> : <Package size={12} />}{part.status === 'sold' ? 'Parduota' : 'Sandėlyje'}</button></td></tr>)}</tbody></table></div> : <div className="paper-line px-6 py-8 text-center"><Package size={24} className="mx-auto text-muted-foreground/50" /><p className="mt-3 text-sm font-semibold">Detalės dar nesurašytos</p><p className="mt-1 text-xs text-muted-foreground">Pridėk pirmą detalę iš šio donoro.</p></div>}</article>;
}
...
function PartsCarModal({ car, close, save }: { car?: PartsCar; close: () => void; save: (payload: Omit<PartsCar, 'id' | 'createdAt' | 'parts' | 'status'> & { id?: string }) => void }) {
  const [form, setForm] = useState({ year: String(car?.year ?? 2015), make: car?.make ?? '', model: car?.model ?? '', engine: car?.engine ?? '', fuel: car?.fuel ?? 'Dyzelinas', mileage: String(car?.mileage ?? ''), purchasePrice: String(car?.purchasePrice ?? '') });
